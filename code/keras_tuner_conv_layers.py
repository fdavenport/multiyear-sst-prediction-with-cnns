...
def model_builder(hp):
    ...
    n_conv_blocks = hp.Choice("n_conv_blocks", [1, 2, 3])
    n_filters = hp.Choice("n_filters", [1, 4, 8, 16, 32, 64, 128, 256, 512])
    
    np.random.seed(101) 
    random.seed(101)
    tf.random.set_seed(101)

    model = mu.build_model(n_conv_blocks = n_conv_blocks, 
                           n_filters = n_filters, 
                           kernels = [(3,3), (3,3), (3,3)], 
                           n_dense_layers = 1, 
                           n_neurons = 32, 
...
tuner = kt.GridSearch(
    ...
    overwrite=False, 
    directory="../processed_data/kt_results/kt_conv",
...
tensorboard_callback = tf.keras.callbacks.TensorBoard(
    log_dir='../processed_data/kt_logs/kt_conv/kt_60mo_'+args.model+'_'+str(args.lat)+'_'+str(args.lon))
