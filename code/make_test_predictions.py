...
test_variants = VARIANT_DICT[args.model_test][test_index]

orig_dates = utils.load_dates()
lats, lons = utils.load_lat_lon()

x_test, y_test = read.load_xy_data(orig_dates, input_length, args.lead, args.prediction_length,
                              X_VAR_NAME = "tos", Y_VAR_NAME = args.y_var, VARIANTS = test_variants, 
...
empty_array = np.full((len(test_variants), N_dates, len(lats), len(lons)), np.nan)
...
for i, (i_lat, i_lon) in enumerate(zip(d_lats, d_lons)): 
    ...
    for j, v in enumerate(test_variants): 
        x = x_test[(j*N_dates):((j+1)*N_dates)]
        
        pred = model.predict(x, verbose=0)
        test_predictions.var1.loc[dict(variant = v, lat = lats[i_lat+5], lon = lons[i_lon])] = pred[:,0]
        test_predictions.var2.loc[dict(variant = v, lat = lats[i_lat+5], lon = lons[i_lon])] = pred[:,1]
        test_predictions.var3.loc[dict(variant = v, lat = lats[i_lat+5], lon = lons[i_lon])] = pred[:,2]

test_predictions.to_netcdf("../processed_data/training/"+args.y_var+"_"+str(args.prediction_length)+\
                  "mo_"+str(args.lead)+"lead_"+args.model_NN+"-trained_"+args.model_test+"-predictions_"+str(args.seed)+".nc")
